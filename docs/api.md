...
### Aggregate Analytics APIs

These endpoints are backed by persisted hourly aggregate snapshots, not raw full-window Parquet scans.

Behavior:
- snapshot base cadence follows `query.dashboard_snapshot_refresh_secs`
- aggregate retention follows `query.analytics_retention_days`
- `refresh=delta` attempts to merge newer finalized Parquet data on top of the latest hourly snapshot
- responses include freshness metadata under `meta`

### `GET /api/analytics/summary`

Returns bounded totals plus top aggregate dimensions.

Query parameters:
- `from` RFC3339 timestamp
- `to` RFC3339 timestamp
- `refresh` optional: `delta`
...
params = {
    ...
    "to": "2026-04-05T12:00:00Z",
    "refresh": "delta",
}
response = require_ok(session.get(f"{BASE_URL}/api/analytics/summary", params=params))
pretty(response.json())
```
...
Returns hourly points for one or more aggregate series.

Query parameters:
- `from` RFC3339 timestamp
- `to` RFC3339 timestamp
- `series` comma-separated list
- `refresh` optional: `delta`
...
- `user:<value>`
- `device:<value>`
- `destination_ip:<value>` or `server_ip:<value>`
- `source_ip:<value>`
- `department:<value>`
...
params = {
    "from": "2026-04-05T08:00:00Z",
    "to": "2026-04-05T12:00:00Z",
    "series": "events,blocked,response_code:403",
}
response = require_ok(session.get(f"{BASE_URL}/api/analytics/timeseries", params=params))
pretty(response.json())
```

### `GET /api/analytics/top`

Returns top values for one aggregate dimension over the requested window.
...
response = require_ok(session.get(f"{BASE_URL}/api/analytics/top", params=params))
pretty(response.json())
```
